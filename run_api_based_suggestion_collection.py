# run_api_based_suggestion_collection.py
...
from src.api_based_suggestion_collector import ApiBasedSuggestionCollector


async def main():
    """メインの実行関数"""
    print("=== APIベース メインKWサジェスト収集システム ===")
    ...
    print("Google Suggest APIとYahoo Suggest APIからサジェストを段階的に収集します。")
    ...
    # ApiBasedSuggestionCollectorの初期化
    collector = ApiBasedSuggestionCollector()
    ...
    try:
        ...
        # JSONファイルに保存
        json_filename = f"{timestamp}_{safe_keyword}_api_suggestions.json"
        ...
        # テキストファイルに保存（サブKW作成用）
        txt_filename = f"{timestamp}_{safe_keyword}_api_suggestions.txt"
        ...
        # フィルタリング済みサジェストも保存
        filtered_txt_filename = f"{timestamp}_{safe_keyword}_api_filtered_suggestions.txt"
