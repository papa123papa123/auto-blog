...
class FastGoogleCollector:
    ...
    async def get_serp_data(self, keyword):
        ...
        try:
            ...
            for t in j.get("tasks", []):
                for res in t.get("result", []):
                    for item in res.get("items", []):
                        if isinstance(item, dict):  # 辞書型かチェック
    ...
    async def collect_step3_fast_expansion(self, keywords, max_per_keyword=2):
        """ステップ3: 高速拡張サジェスト取得（時間短縮版）"""
        print(f"\n🔍 ステップ3: 高速拡張サジェスト取得中... ({len(keywords)}件のキーワードから)")
        print(f"   ⚡ 高速化: 各キーワードから最大{max_per_keyword}件のみ取得")
        ...
        for i, keyword in enumerate(target_keywords):
            print(f"  📍 処理中: {keyword} ({i+1}/{len(target_keywords)})")
            
            # 各キーワードから最大2件のサジェスト取得
    ...
    async def run_collection(self, seed_keyword, target_count=100):
        ...
        if len(self.all_keywords) < target_count:
            ...
            expanded = await self.collect_step3_fast_expansion(base_keywords)
        else:
            expanded = []
        ...
        print(f"   - ステップ1 (シードサジェスト): {len(seed_suggestions)}件")
        print(f"   - ステップ2 (SERP関連): {len(related) + len(paa)}件")
        print(f"   - ステップ3 (高速拡張): {len(expanded)}件")
        ...
        return {
            ...
            "raw_data": {
                "step1_seed_suggestions": seed_suggestions,
                "step2_serp_related": related,
                "step2_serp_paa": paa,
                "step3_expanded": expanded
            },
            "breakdown": {
...
async def main():
    ...
    async with FastGoogleCollector() as collector:
        result = await collector.run_collection(seed_keyword, target_count)
        
        if result:
            # 結果をファイルに保存
            output_file = f"fast_collected_{seed_keyword}_{len(result['keywords'])}件.json"
