...
class AgentCollaborationGoogleCollectorFixed:
    def __init__(self):
        self.client = None
...
async def main():
    ...
    async with AgentCollaborationGoogleCollectorFixed() as collector:
        result = await collector.run_agent_collaboration_collection(seed_keyword, target_count)
        
        if result:
            # 結果をファイルに保存
            output_file = f"agent_collaboration_fixed_{seed_keyword.replace(' ', '_')}_{len(result['keywords'])}件.json"
