# run_final_operation.py
"""
【最終版】「スクロール4枚撮り」と「AIによる統合・清書」を実行し、
...
async def main():
    ...
    from final_screenshot_taker import take_screenshots
    from final_spec_extractor import FinalSpecExtractor
    ...
    # 1. スクリーンショット撮影
    await take_screenshots(TARGET_CATEGORY_URL, TARGET_CATEGORY_NAME)
    ...
    for group_name, img_paths in image_groups.items():
        ...
        if len(img_paths) == 4:
            ...
            all_extracted_texts.append(f"--- 比較グループ: {group_name} ---\n\n{extracted_text}\n\n")
        else:
            print(f"[WARN] 画像が4枚揃っていないため、グループ '{group_name}' をスキップします。")

    # 3. 最後に、すべてのテキストを一つのファイルに統合
    ...
    # ★★★ 目立って分かりやすい名前 ★★★
    output_filename = f"【統合版】{TARGET_CATEGORY_NAME}_スペック比較_{timestamp}.txt"
