...
class AgentCollaborationGoogleCollectorFinal:
    def __init__(self):
        self.client = None
                ...
                for res in t.get("result", []):
                    for item in res.get("items", []):
                        ...
                        if item_type == "related_searches":
                            # Related Searchesの正しい構造（文字列のリスト）
                            if "items" in item and isinstance(item["items"], list):
                                for rel_item in item["items"]:
                                    if isinstance(rel_item, str):
                                        related_searches.append(rel_item)
                        ...
                        elif item_type == "extended_people_also_search":
                            # Extended People Also Searchの正しい構造
                            if "items" in item and isinstance(item["items"], list):
                                for paa_item in item["items"]:
                                    if isinstance(paa_item, dict):
                                        question = paa_item.get("question") or paa_item.get("text")
                                        if question:
                                            paa_questions.append(question)
...
"""
    ...
    def get_agent_selection(self, filename):
        ...
        print(f"📋 ファイル {filename} の分析結果を参考にしてください")
            ...
            for i, task_result in enumerate(j.get("tasks", [])):
                ...
                for res in task_result.get("result", []):
                    for item in res.get("items", []):
                        ...
                        elif item_type in ["people_also_ask", "extended_people_also_search"]:
                            if "items" in item and isinstance(item["items"], list):
                                for paa_item in item["items"]:
                                    if isinstance(paa_item, dict):
                                        question = paa_item.get("question") or paa_item.get("text")
...
async def main():
    ...
    async with AgentCollaborationGoogleCollectorFinal() as collector:
        result = await collector.run_agent_collaboration_collection(seed_keyword, target_count)
        
        if result:
            # 結果をファイルに保存
            output_file = f"agent_collaboration_final_{seed_keyword.replace(' ', '_')}_{len(result['keywords'])}件.json"
